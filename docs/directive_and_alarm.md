...
| plugin_id    | `plugin_id` of the rule                                                                                      | integer from 1                                                                                                                                                                                                                                                                                                                                                                                                           | Yes, together with `plugin_sid` in a `PluginRule`                 | Only if `product` and `category` is empty                  |
| plugin_sid   | `plugin_sid` of the rule                                                                                     | array of integers, e.g. `[1,2,5,6]`                                                                                                                                                                                                                                                                                                                                                                                      | Yes, together with `plugin_id` in a `PluginRule`                  | Only if `product` and `category` is empty                  |
| product      | `product` field of the rule                                                                                  | array of strings, e.g. `["iptables","nftables"]`                                                                                                                                                                                                                                                                                                                                                                         | Yes, with `category` in a `TaxonomyRule`                          | Only if `plugin_id` is 0 or `plugin_sid` is an empty array |
| category     | `category` field of the rule                                                                                 | strings, e.g. `"firewall"`                                                                                                                                                                                                                                                                                                                                                                                               | Yes, with `category` in a `TaxonomyRule`                          | Only if `plugin_id` is 0 or `plugin_sid` is an empty array |
...
| from         | How `SRC_IP` of the event will be matched                                                                    | This can be `ANY` (will match any IP address), `HOME_NET` (will match those defined in assets.json), `!HOME_NET`(will match those that aren't defined in assets.json), specific IPv4/IPv6 address, CIDR notation, or a list of addresses separated by comma. Each address can also be prefixed with `!` to specify negation. Examples: `"192.168.0.1"`, `"192.168.0.0/24"`, `"!10.0.0.1", "192.168.0.0/24, 10.0.0.0/8" ` | Yes                                                               | Yes                                                        |
| to           | How `DST_IP` of the event will be matched                                                                    | The Same format with `from` above                                                                                                                                                                                                                                                                                                                                                                                        | Yes                                                               | Yes                                                        |
| port_from    | How `SRC_PORT` of the event will be matched                                                                  | This can be `ANY`, a specific port number (0-65535), or a list of port numbers separated by comma                                                                                                                                                                                                                                                                                                                        | Yes, if not `0`                                                   | No, defaults to `0`                                        |
| protocol     | How `protocol` of the event will be matched                                                                  | can be `ANY` or a specific string, e.g. `"ICMP"`, `"HTTP"`                                                                                                                                                                                                                                                                                                                                                               | Yes                                                               | No                                                         |
| custom_data1 | How `custom_data1` of the event will be matched                                                              | can be `ANY`, or a specific string, e.g. `"access denied"`, `"accepted"`                                                                                                                                                                                                                                                                                                                                                               | Yes                                                               | No                                                         |
| custom_data2 | How `custom_data2` of the event will be matched                                                              | can be `ANY`, or a specific string, e.g. `"access denied"`, `"accepted"`                                                                                                                                                                                                                                                                                                                                                               | Yes                                                               | No                                                         |
| custom_data3 | How `custom_data3` of the event will be matched                                                              | can be `ANY`, or a specific string, e.g. `"access denied"`, `"accepted"`                                                                                                                                                                                                                                                                                                                                                               | Yes                                                               | No                                                         |
| reliability | Determines the directive reliability value once the rule completes                                           | integer 0 to 10                                                                                                                                                                                                                                                                                                                                                                                                          | No                                                                | Yes                                                        |
| timeout     | the rule maximum active duration (in seconds) since the timestamp of the first matching event                | integer starting from 0                                                                                                                                                                                                                                                                                                                                                                                                  | No                                                                | defaults to 0 |

Some of the fields can also be set to refer to value from the _first event_ of a _previous stage_ using a `:[stage]` notation.

For example, Let's say a rule in stage 3 has its `from` set to `:2`. When the directive processing reaches stage 3, `SRC_IP` of incoming events will be matched against the `SRC_IP` of the _first event_ that matches the stage 2 rule. This allows later stages to inherit some conditions from previous stages as part of its own rule conditions.
