...
pub fn gen_sidpairs(correct_pair: SIDPair, n: u64, plugin_id: Range<u64>, plugin_sid: Range<u64>) -> Vec<SIDPair> {
    let mut pairs = vec![];
    for i in 0..n {
        ...
        pairs.push(SIDPair { plugin_id, plugin_sid });
    }
...
pub fn gen_taxopairs(correct_pair: TaxoPair, n: u64) -> Vec<TaxoPair> {
    ...
    for i in 0..n {
        let product: Vec<String> = if i < n / 2 {
            vec![rand::thread_rng().sample_iter(&Alphanumeric).take(10).map(char::from).collect()]
        } else {
