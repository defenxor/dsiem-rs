...
const MAX_CHECKS_PER_THREAD: usize = 3_000_000;
...
impl Manager {
    ...
    pub fn start(&self, report_interval: u64) -> Result<()> {
        ...
        let mut dirs_per_thread = MAX_CHECKS_PER_THREAD / self.option.max_eps as usize;
        if dirs_per_thread < 100 {
            dirs_per_thread = 100;
        }
        let max_threads = available_parallelism()?.get() / 2;
        let dir_len = dir_params.len();

        if dir_len / dirs_per_thread > max_threads {
            return Err(anyhow!("too many directives ({}) for available parallelism ({}), reduce the number of directives or max_eps and try again", dir_len, max_threads));
        }

        let r = dir_params.chunks(dirs_per_thread).collect::<Vec<_>>();
        let mut chunks = vec![];
        for c in r {
            chunks.push(c.to_owned());
        }

        info!("backlog manager started with max single event processing time: {} ms, queue limit: {} events, quick check threads: {}, ttl directives: {}",
        ...
        let dir_managers_threads = available_parallelism()?.get() - chunks.len() - 1;

        let h_managers = self.spawner(dir_managers, dir_managers_threads);
